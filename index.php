...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['route']) && $_GET['route'] === 'inscription_tournoi') {
    // Récupération des données nécessaires
    $tournoi_id = $_POST['tournoi_id']; // Assurez-vous que le formulaire a un champ caché avec cet ID

    // Insertion dans la base de données
    $stmt = $pdo->prepare("INSERT INTO InscriptionTournoi (utilisateur_id, tournoi_id) VALUES (?, ?)");
    if ($stmt->execute([$_SESSION['user_id'], $tournoi_id])) {
        // Redirection ou message de succès
        header('Location: index.php?route=tournois'); // Redirection vers la liste des tournois
        exit;
    } else {
        echo "<p>Erreur lors de l'inscription au tournoi. Veuillez réessayer.</p>";
    }
}
...
<div class="wrapper">
    ...
    <main>
        ...
        switch ($route) {
            ...
            case 'create_tournament':
                ...
                if (isset($_GET['route']) && $_GET['route'] === 'create_tournament') {
            ...
            case 'manage_tournois':
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    if ($_POST['action'] === 'créer') {
                        $stmt = $pdo->prepare("INSERT INTO Tournoi (nom, description, date, statut) VALUES (?, ?, ?, ?)");
                        $stmt->execute([$nom, $description, $date, $statut]);
                    } elseif ($_POST['action'] === 'modifier') {
                        $stmt = $pdo->prepare("UPDATE Tournoi SET nom = ?, description = ?, date = ?, statut = ? WHERE id = ?");
                        $stmt->execute([$nom, $description, $date, $statut, $id]);
                    ...
                    include 'views/manage_tournois.php';
                }
                break;
    ...
    <script src="script.js"></script> <!-- Ajoutez votre fichier JavaScript ici -->
